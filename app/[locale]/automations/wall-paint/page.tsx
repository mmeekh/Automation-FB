...
import { getWallPaintContent } from '@/lib/automation-content';
import { useAutomationLauncher } from '@/lib/hooks/useAutomationLauncher';
import type { Locale } from '@/i18n/config';

type WallPaintPageProps = {
  params: { locale: Locale };
};

export default function WallPaintPage({ params }: WallPaintPageProps) {
  const launchAutomation = useAutomationLauncher();
  const content = getWallPaintContent(params.locale);
  const stats = content.stats ?? [];
  const howItWorks = content.howItWorks;

  const handleLaunch = () => {
    launchAutomation('wall-paint');
  };
