...
const CATEGORY_EMOJI_PLACEHOLDER: Record<AutomationTemplate['category'], string> = {
  'Aesthetic AI': '💎',
  Automotive: '🚗',
  'Car Color Changer': '🎨',
  Fashion: '👗',
...
export function CollapsedView({
  ...
  const getPlaceholderEmoji = (type: 'before' | 'style' | 'result') => {
    ...
    return CATEGORY_EMOJI_PLACEHOLDER[template.category] ?? FALLBACK_PLACEHOLDER;
  };

  const renderPreview = (type: 'before' | 'style' | 'result') => (
    <div className="flex flex-col items-center gap-1">
      <div
        className={`relative w-20 h-20 rounded-xl overflow-hidden border-2 ${
          type === 'result' ? 'border-accent-300' : 'border-neutral-200'
          ...
          <div
            className={`absolute inset-0 ${
              type === 'style'
                ...
                : type === 'result'
                ? 'bg-gradient-to-br from-accent-100 via-primary-100 to-accent-200'
                : 'bg-gradient-to-br from-neutral-100 to-neutral-200'
            } flex items-center justify-center`}
          >
            <span className="text-2xl">{getPlaceholderEmoji(type)}</span>
          </div>
        )}
      </div>
      ...
      <div className="p-6">
        ...
        <div className="mb-6">
          <div className="flex items-center justify-center gap-2">
            ...
            <span className="text-2xl font-bold text-primary-600 transform group-hover:rotate-180 transition-transform duration-500 mb-3">
              +
            </span>
            {renderPreview('style')}
            <span className="text-2xl font-bold text-accent-600 transform group-hover:scale-110 transition-transform duration-500 mb-3">
              =
            </span>
        ...
        <div className="mb-5 text-center">
          <h3 className="text-lg font-bold text-neutral-900 mb-1.5 leading-tight">{template.name}</h3>
          <p className="text-xs text-neutral-600 leading-relaxed line-clamp-2">{template.description}</p>
        </div>
